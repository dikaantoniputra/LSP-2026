    ...
    <div class="col-12 col-md-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center py-3">
                <h5 class="mb-0 fw-bold">Tambah Data Produk</h5>
                ...
                <form action="{{ route('products.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="status" id="statusTersedia" value="Tersedia" {{ old('status', 'Tersedia') === 'Tersedia' ? 'checked' : '' }}>
                            <label class="form-check-label" for="statusTersedia">Tersedia</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="status" id="statusHabis" value="Habis" {{ old('status') === 'Habis' ? 'checked' : '' }}>
                            <label class="form-check-label" for="statusHabis">Habis</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="status" id="statusPreorder" value="Preorder" {{ old('status') === 'Preorder' ? 'checked' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="deskripsi" id="deskripsi" rows="3" class="form-control @error('deskripsi') is-invalid @enderror" placeholder="Keterangan singkat produk...">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
